...
† `xhigh` is **now in the effort vocabulary** (committed `fb1b80e`), so these profiles are directly executable; **`high`** is a valid alternative. Implementor and verifier are **different vendors** (xAI-via-OpenCode builds, OpenAI/Codex verifies) with Anthropic/Claude coordinating — the exact cross-harness independence the harness exists to provide, now turned on itself. The verifier is read-only on the implementor's exact commit; the coordinator writes no files.
