...
**Write model — LOCKED: single-writer daemon with CLI forwarding.** The daemon holds an **exclusive lifetime writer lock** (the daemon lockfile, §3.8) and appends all events in-process, notifying subscribers post-commit. When a daemon is running, the CLI **forwards** its command to the daemon over loopback rather than writing directly.

- **Direct CLI writes require ATOMIC proof of no live writer:** a failed HTTP connection does **not** prove the daemon is absent. The CLI may write directly **only after it atomically acquires/reclaims that same exclusive writer lock** (§14 identity — unclaimed, or a provably dead/recycled owner). A **live-but-unreachable** daemon (lock held by a live owner) makes the CLI **retry or fail** — it never silently becomes a second writer.
...
- **Seams — to build:** (b) multi-run **enumeration** read model (no `listRuns` today) — **filter to rows with a real `RUN_META` projection** and EXCLUDE the RunId-shaped system scopes that share the runs registry (`run__process_registry`, `run__desired_model`, `run__run_ownership`, `run__spawn_reservations`, `run__failover_incident`), or add an explicit real/system `kind` discriminator; never enumerate every projection-scope as a user run; (a) HTTP + multiplexed WS on loopback with **port discovery** (not a hard-coded `7717`); (c) event delivery via the **locked single-writer model** (§3.7) — in-process post-commit notify + CLI command-forwarding, gated by the daemon's exclusive writer lock; (d) the durable **attention** queue; per-run **execution contexts** (§3.6); the **security gate** (§3.8); daemon **lockfile** + stale reclaim.
...
- **Daemon lifecycle — LOCKED (§3A.6):** a normal child sidecar dies with the app, which would break "quit UI, keep service running", so:
  ...
  - **Production = a managed background helper / LaunchAgent** (macOS) that owns the daemon **independently of the UI**, so runs survive with no Electron process at all; the UI connects via the §3.8 connection-metadata + writer lock. (The earlier "detached child" option is superseded by these two.)
