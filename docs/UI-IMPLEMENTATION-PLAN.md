...
**Revision 12 — engine-state vocabulary completeness (blocking input for slices 4 / 6a):** the state vocabulary this plan inherited from the prototype is **incomplete against the engine**, and a UI built from it would render real runs blank.

- **Suspension.** The engine defines **six** `SUSPENSION_KINDS` (`src/domain/state.ts:39`): `none · paused_limit · paused_user · breaker_open · interrupted · resource_exhausted`. §1.5 enumerates four and flags `paused_user` as a known design omission; **`resource_exhausted` appears nowhere in this plan or in `UI-DESIGN-BRIEF.md`.** It is not exotic — it is the F1/F3 RSS-budget kill (a generation crossed its memory ceiling and was terminated), and it is semantically distinct from `paused_limit`: a local memory ceiling has **no provider ETA**, and resume is gated on an *audited per-run budget raise* (`set-budget RUN_ID --role <role> --memory-budget-mb <MB> --resume` — `--role` is required, `args.ts:61`), never an automatic escalation. A suspension banner that assumes "wait for a reset time" is simply wrong for this state.
- **Terminal outcomes.** `provisioning_failed` (dependency provisioning failed closed — no verifier dispatch, no `merge_ready`) and `no_deliverable` (the implementor produced no new commit) also appear **zero** times in either UI doc. Two engine nuances the failure screens must encode: `no_deliverable` is **not** a `LoopOutcome` — it surfaces as a thrown `NoDeliverableError` from `runRole` (`service.ts:2694`) — and `merge_ready` is deliberately **not** in `TERMINAL_PHASES` (`state.ts:29`), so `cancel` remains legal from it.

**Action:** the slice-4 (shell + read screens) and slice-6a (failure screens) specs must carry the complete vocabulary — all six suspension kinds including `paused_user` and `resource_exhausted`, plus both terminal outcomes with their distinct operator actions — before those slices are drafted. Nothing else in this plan changes; the screen inventory, contracts and phase ordering are untouched.
