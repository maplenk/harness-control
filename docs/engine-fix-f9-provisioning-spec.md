...
New proof, executed in the **STAGED** tree **before the marker write at `provision.ts:589`** (so an unproven tree can never become sticky): derive from the fingerprinted manifests the list of installed packages declaring `install`/`postinstall` scripts (at minimum better-sqlite3 today), and run a bounded, allowlisted-env probe for each from the staged tree root. Any failure → `provisioning_failed`, cause `native_toolchain_unproven`, **naming the package**.

**The probe must EXECUTE the package, not merely `require()` it.** A bare `require('better-sqlite3')` can succeed against a tree with no built binding, because the native binding is resolved lazily when a database is first constructed — so `require`-only would reproduce the very false-proof this fix exists to kill. The probe for better-sqlite3 therefore opens an in-memory database and runs a trivial query, and asserts the result. Generalize this as a small table of **per-package executable probes**, with `require()` accepted as a fallback only for packages that have no defined probe (and that fallback should be treated as weak evidence, not proof — any package whose failure mode is lazy deserves a real probe). The probe runs on **both** lanes before marker creation (unchanged).
...
- **AC-5b the probe is executable, not `require`-only** — on that same unbuilt tree, assert that a bare `require()` of the package would have SUCCEEDED (or, if it fails on this version, that the probe does not *depend* on it failing) while the executable probe FAILS. This is the regression guard for the lazy-binding hole: a probe that only imports would pass the exact tree this AC exists to reject.
