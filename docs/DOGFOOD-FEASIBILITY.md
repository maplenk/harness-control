...
**L11 — Preflight battery before EVERY run, and it must drill the ENGINE.** Born from the two 2026-07-25 misses (the grok permission trap and F10), both invisible to a green suite. A scripted, sub-minute, zero-spend dry-run that exercises the engine's REAL paths against the CURRENT tooling before any coordinator dollar. Shipped as **`scripts/dogfood/preflight.sh`**.

**It is ADVISORY, not enforced — you have to remember to run it.** An enforcement gate (binding the record to HEAD, the dist digest, the resolved roles/config, the canonical store/log paths and a durable attempt claim) was built and reviewed to convergence, then **deliberately deferred to the `gate-enforcement` branch**. The reason is worth recording: each review round of that machinery added surface that the next round then reviewed — 10 → 7 → 5 → 5 → 6 findings, not converging — while the thing actually on the critical path (the engine cannot commit on git 2.55, F10) waited behind it. Enforcement is process tooling; it is not what unblocks the engine. The trade accepted: every slice is run personally, from a written runbook, and the runbook starts with preflight.

The load-bearing design rule, learned the hard way: **a simulation of what the engine is believed to do can pass while the engine fails.** The first version of section (d) hand-rolled a `git add` that mimicked the staging helper — and it passed, on a machine where the engine's own helper throws. Section (d) now imports the **real** `addAllExceptNodeModules` out of `dist/` and runs it against an ignored+present `node_modules` fixture, with a vacuous-pass guard so "nothing staged" cannot read as success. The hand-rolled version survives only as a labelled, non-gating canary that explains *why* a real-helper failure happened. That single check is the reason the battery is worth running at all: it is the difference between "the suite is green" and "this machine can produce a commit".

Sections: **(a)** toolchain provenance — git/node/npm recorded as a JSON line in `$HARNESS_HOME/logs/preflight.jsonl`, drift vs `preflight-baseline.json` warns (the run fingerprint's `platformKey` covers node only, and F10 was a **git**-version interaction it cannot see); **(b)** runtime toolchain proof — better-sqlite3 opened *and queried*, `tsc`/`vitest` executed, because a populated `.bin` proves nothing (F9/P1); **(c)** `npm run build` + `doctor --json` with the pinned dogfood config, evaluated **per dispatched role** (claude/grok/codex fail the battery; unused adapters are printed but never fail it — a blanket accept of `overall:"warn"` would swallow a broken grok); **(d)** the real-helper staging drill described above; **(e)** the L8 discovery floor — `vitest list --filesOnly` is a root-reroute canary, not a real collection, and the floor may only be raised above the committed `floorMin`, never lowered; **(f)** clean tree (the F5 base-pin precondition) plus a sha256 digest of the whole `dist/` tree, run last so it also re-proves the battery left no dirt. The store and log dir must resolve **outside** the repo, checked before anything is created — by the battery and independently by each slice wrapper, because `mkdir -p` on an unvetted path is what put directories inside the working tree before the refusal could fire.

**The kept surface is tested.** `scripts/dogfood/preflight-selftest.sh` (`npm run test:preflight`) is checked in and runs from a clean checkout: 25 assertions over the digest primitives (content, empty directories, symlinked child, symlinked root, empty tree), containment (absolute and relative `symlink/..` shapes that land in the repo), the wrappers refusing before they create anything, and **one real end-to-end `preflight.sh` run** proving the staging drill fires and records provenance. It deliberately does not synthesize records to assert on — those fixtures belonged to the deferred gate, and a suite that mostly exercises its own fixtures is theatre.

**⚠ Observation is NOT side-effect-free.** Every CLI invocation carrying a run id delivers pending alerts and appends `alert.delivered` to the durable log (`commands.ts:201` → `service.ts:1658`, deliberate P4b-1 at-least-once delivery). That includes plain `status`, and therefore `monitor.sh`, which polls it: monitoring **writes** to the event log, advances sequence numbers, and takes the SQLite write lock. `watch.sh` — direct `sqlite3 -readonly` queries — is the only truly read-only way to watch a run. This also means `resume`, `recheck` and `set-budget --resume` are ordinary spending paths: re-run preflight before them, exactly as before `run`.
