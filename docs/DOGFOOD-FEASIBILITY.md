...
**L12 — A criterion's success exit code must be declared, and `!` is not a substitute.** Born from run `run_60ccbfda` (2026-07-25), the first run to reach `verifying`. It returned `failedCriteria: []` and five `unproven` — nothing wrong with the code. Four of the five pass by a `grep` finding **nothing**, which exits `1`, and F13's host-attestation gate requires every declared command to exit `0` (`verifier.ts:876`). **The engine therefore cannot prove ABSENCE** — which is the shape of every scope, isolation and containment criterion, i.e. exactly the checks the harness exists to enforce. → **F15 filed** (`docs/specs/f15-declared-exit-codes-spec.md`): let a criterion declare its expected exit code, defaulting to `0`.

**F15 has landed.** Declare the code: a bare command string still means "must exit `0`", and `{ "command": "…", "expectedExitCode": N }` declares any other proving code — mandatory for every absence check, since `grep` exits `1` when it finds nothing. Exit `124` is reserved by the host for timeout/output-cap terminations and is rejected by the §7 validator. **Do NOT wrap a check in `!`**: `! grep -R pattern path` exits `0` both when the pattern is absent *and* when grep failed with exit `2` (unreadable path, bad regex) — converting "I could not determine X" into "X is false", the same bug in the opposite polarity. Where the distinction matters, use an explicit form that names the code (`grep …; test $? -eq 1`) rather than an inversion that swallows it.
