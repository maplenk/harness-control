...
**The gate is itself tested.** `scripts/dogfood/preflight-selftest.sh` (`npm run test:preflight`) is checked in and runs from a clean checkout: 42 assertions across record freshness/identity, completeness, digest/role/config binding, the attempt claim, store-log containment, ordering, the lib primitives, and the never-leave-an-older-PASS invariant. Each asserts the exit code *and* the specific refusal text, builds its fixtures in a scratch dir outside the repo, and never touches the real `$HARNESS_HOME`. A gate whose own guarantees cannot be re-run is not a gate — this exists because an earlier round's coverage lived only in a session transcript.

**Every spending path is gated, not just `run`.** `resume`, `recheck` and `set-budget --resume` all drive provider turns and mutate a run, and they are what an operator reaches for when a run is *already* in trouble. They go through `scripts/dogfood/resume-slice.sh`, which runs the containment refusal and the gate (with the run id, so the config binding comes from the run's persisted config) before touching the CLI. Read-only observation (`status`, `monitor.sh`, `watch.sh`) needs no gate.
